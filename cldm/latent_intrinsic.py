...
class UNet(torch.nn.Module):
    def __init__(self,
        ...
        block_kwargs['emb_channels'] = emb_channels
        # Decoder.
...
class LatentIntrinsc(torch.nn.Module):
...
#             model_path = hf_hub_download(repo_id='xyxingx/LumiNet', filename='last.pth.tar')
        ...
        new_dict = {}
        for k, value in checkpoint['ema_state_dict'].items():
            key = k.split('module.')[-1]
            new_dict[key] = value
        ...
        self.model_latents = model
        self.concat = True
    
    def forward(self,x):
        with torch.no_grad():
            intrinsic_input, extrinsic_input = self.model_latents(x[:,:3,...],run_encoder=True)
            intrinsic_ref, extrinsic_ref = self.model_latents(x[:,3:,...],run_encoder=True)
        intrinsic_input_o = intrinsic_input[2]
            # non-litencoding & pos-encoding
            # extrinsic_ref = extrinsic_ref.unsqueeze(2).unsqueeze(3)
            # extrinsic_ref = extrinsic_ref.repeat(1,1,32,32)
            # prior_hints = torch.cat([intrinsic_input_o,extrinsic_ref],dim=1)
            #light-encoding & pos-encoding
        extrinsic_ref_cat = extrinsic_ref+0
        extrinsic_ref = extrinsic_ref.repeat(1,192)
        extrinsic_ref = self.light_decoder(extrinsic_ref)
        extrinsic_ref = extrinsic_ref.unsqueeze(2)
        extrinsic_ref = extrinsic_ref.reshape(-1,3,1024)
        # extrinsic_ref = extrinsic_ref.repeat(1,1,1024)
        # print(extrinsic_ref.shape)
        # exit(0)
        extrinsic_ref_cat = extrinsic_ref_cat.unsqueeze(2).unsqueeze(3)
        extrinsic_ref_cat = extrinsic_ref_cat.repeat(1,1,64,64)
        
        # extrinsic_ref = extrinsic_ref.repeat(1,1,1024)
        prior_hints = torch.cat([intrinsic_input_o,extrinsic_ref_cat],dim=1)
        # else:
        #     emb = self.light_decoder(extrinsic_ref)
        #     params = emb.unsqueeze(2).unsqueeze(3).to(intrinsic_input_o.dtype).tanh() * 3e-5
        # # params = self.affine(emb).unsqueeze(2).unsqueeze(3).to(x.dtype).tanh() * self.affine_scale
        #     prior_hints = torch.nn.functional.silu(intrinsic_input_o * (params + 1))
        # print("here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # print(prior_hints.shape)
        return prior_hints,extrinsic_ref